...
def get_contexts_ds( level, mask_valid):
    ...
    templete_contexts = templete_contexts.to(device)
...
def compute_hdn_loss(SSI_LOSS,depth_preds,depth_gt,mask_valid_list):
    ...
    hdn_loss = hdn_loss_level_list.sum() / mask_valid_list_times.sum() * 3  # average the losses of all locations


    return hdn_loss

def hdn_total_loss(depth_preds, depth_gt, mask_valid):
    ...
    return (loss_hdn_dr + loss_hdn_dp + loss_hdn_ds) / 3
